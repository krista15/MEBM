...
% feedbacks include CMIP5 mean feedback and Rose et al feedbacks
...
feedback = interp1(CMIP5_lat,mean(CMIP5_lambda,2),phi,'linear');
...
% arctic bump
feedback = -4*ones(size(x));
x0 = 0.90;dx0 = 0.15;
feedback = feedback - (feedback+1.5).*exp(-((x-x0)/dx0).^4);
feedback = feedback - (feedback+1.50).*exp(-((x+x0)/dx0).^4); 
x0 = 0.1;dx0 = 0.1;
feedback = feedback - (feedback+1.5).*exp(-((x-x0)/dx0).^2); 
feedback = feedback - (feedback+1.5).*exp(-((x+x0)/dx0).^2);
x0 = 0.5;dx0 = 0.2;
feedback = feedback - (feedback+1.5).*exp(-((x-x0)/dx0).^4); 
feedback = feedback - (feedback+1.5).*exp(-((x+x0)/dx0).^4);
...
xlabel('Latitude')
ylabel('W m^{-2} K^{-1}')
title('Rose et al. 2xCO2 feedback')
ax = gca;
set(gca,'XTick',xt); set(gca,'XTicklabel',xtl);
...
feedback = -2.5*ones(size(x));
x0 = 0.95;dx0 = 0.2;
feedback = feedback - (feedback+1.5).*exp(-((x-x0)/dx0).^2);
feedback = feedback - (feedback+1.5).*exp(-((x+x0)/dx0).^2); 
x0 = 0.1;dx0 = 0.15;
feedback = feedback - (feedback+0.1).*exp(-((x-x0)/dx0).^2); 
feedback = feedback - (feedback+0.1).*exp(-((x+x0)/dx0).^2);
x0 = 0.5;dx0 = 0.2;
feedback = feedback - (feedback+0).*exp(-((x-x0)/dx0).^4); 
feedback = feedback - (feedback+0).*exp(-((x+x0)/dx0).^4);
...
xlabel('Latitude')
ylabel('W m^{-2} K^{-1}')
title('Rose et al. high-lat uptake feedback')
ax = gca;
set(gca,'XTick',xt); set(gca,'XTicklabel',xtl);
...
feedback = -10*ones(size(x));
x0 = 0.1;dx0 = 0.15;
feedback = feedback - (feedback+4).*exp(-((x-0.12)/0.3).^2); 
feedback = feedback - (feedback+4).*exp(-((x+0.12)/dx0).^2);
x0 = 0.8;dx0 = 0.2;
feedback = feedback - (feedback+2).*exp(-((x+x0)/0.3).^4);
x0 = 0.95;dx0 = 0.1;
feedback = feedback - (feedback-3).*exp(-((x+0.95)/0.08).^2);
feedback = feedback - (feedback+2).*exp(-((x-1)/0.4).^4);
...
xlabel('Latitude')
ylabel('W m^{-2} K^{-1}')
title('Rose et al. tropical uptake feedback')
ax = gca;
set(gca,'XTick',xt); set(gca,'XTicklabel',xtl);
